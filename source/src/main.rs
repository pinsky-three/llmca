...
use source::gemma3_loader::{self, ModelWeightsGemma3};
use std::io::Write;
...
impl Args {
    ...
    fn model(&self) -> anyhow::Result<std::path::PathBuf> {
        ...
        Ok(std::path::PathBuf::from(
            "/home/bregy/Downloads/gemma-3-1b-it-Q4_K_M.gguf",
        ))
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    let mut model = match model_path.extension().and_then(|v| v.to_str()) {
        Some("gguf") => {
            ...
            ModelWeightsGemma3::from_gguf(model, &mut file, &device)?
        }
        Some("ggml" | "bin") | Some(_) | None => {
            ...
            panic!("ggml files are not supported yet");
        }
    };
    println!("model built");
    ...
    for prompt_index in 0.. {
        ...
        println!("prompt tokens 1: {}", tokens.get_ids().len());
        ...
        println!(
            "prompt tokens 2: {} ({} bytes)",
            prompt_tokens.len(),
            format_size(prompt_tokens.len() * std::mem::size_of::<u32>())
        );
        ...
        println!("prompt tokens 3: {:?}", prompt_tokens);
        ...
        let mut next_token = if !args.split_prompt {
            ...
            println!("compiling logits");

            let logits = model.forward(&input, 0)?;

            println!("compiling logits done");
